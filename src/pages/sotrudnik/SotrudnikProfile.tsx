...
export function SotrudnikProfile() {
        ...
        <Card>
          <div className="flex flex-col items-center text-center py-4">
            ...
            <div className="flex gap-2 mt-3">
              <Badge variant="purple">Sotrudnik</Badge>
              <Badge variant={profile.status === 'active' ? 'green' : 'red'}>
                {profile.status === 'active' ? 'Aktiv' : 'Bloklangan'}
              </Badge>
            </div>
            <p className="text-xs text-slate-400 mt-2">Qo'shilgan: {formatDate(profile.created_at, 'dd.MM.yyyy')}</p>
          </div>
        </Card>

        <Card>
          <h3 className="font-semibold text-slate-900 dark:text-white mb-4">Ma'lumotlarni tahrirlash</h3>
