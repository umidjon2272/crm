...
import { Plus, Search, Store as StoreIcon, MapPin } from 'lucide-react'
...
function AddStoreModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.store_name || !form.company_name) {
      toast.error('Magazin nomi va firma nomi majburiy')
      return
    }
    setLoading(true)
    try {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            <Button type="button" variant="secondary" isLoading={geoLoading} onClick={handleGeo} leftIcon={<MapPin className="w-4 h-4" />}>
              Lokatsiyani olish
            </Button>
            {form.latitude && (
            ...
            {filtered.map((store) => (
              <Card key={store.id} className="hover:shadow-md transition-shadow">
                ...
                <div className="mt-4 space-y-1.5 text-sm text-slate-600 dark:text-slate-400">
                  ...
                  {store.latitude && <p className="text-xs text-green-600">✓ GPS saqlangan</p>}
                </div>
